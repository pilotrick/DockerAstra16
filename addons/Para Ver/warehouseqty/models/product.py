...
class ProductTemplate(models.Model):
    ...
    def _get_warehouse_quantity(self):
        for record in self:
            ...
            product_id = record.env['product.product'].search([('product_tmpl_id', '=', record.id), '|', ('active', '=', True), ('active', '=', False)])
            if product_id:
                quant_ids = record.env['stock.quant'
                                       ].sudo().search([('product_id', '=',
                        product_id[0].id), ('location_id.usage', '=',
                        'internal')])
                t_warehouses = {}
                ...
                for location in t_warehouses:
                    ...
                    while not warehouse and location1:
                        warehouse_id = record.env['stock.warehouse'
                                ].sudo().search([('lot_stock_id', '=',
                                location1.id)])
                    ...
                    if warehouse_id:
                        ...
                        tt_warehouses[warehouse_id.name] += \
                            t_warehouses[location]
