...
def migrate_sale_invoice_fields(env, company):
    ...
    sales_journal = env["account.journal"].browse([i[0] for i in env.cr.fetchall()])
    ...
    for journal in sales_journal:
        ...
        env.cr.execute(
            """
        update account_journal set l10n_latam_use_documents =  True
        WHERE id = %s
        """
            % journal.id
        )
...
def migrate_partner_fields(env):
    ...
    if env.cr.fetchone()[0] or False:
        ...
        for i, expense_type in env.cr.fetchall():
            ...
            partner_id._write({"l10n_do_expense_type": expense_type})
