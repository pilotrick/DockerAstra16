...
export function usePartnerAutocomplete() {
            ...
            const concatResults = () => {
                ...
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.vat) < 0) {
                            websites.push(suggestion.vat);
    ...
    function enrichCompany(company) {
        return orm.call(
            'res.partner',
            'dgii_enrich_company',
            [company.website, company.partner_gid, company.rnc]
        );
    }
    ...
    function getCreateData(company) {
        const removeUselessFields = (company) => {
            ...
            const fields = [
                'label', 'commercial_name', 'rnc', 'result_number', 'payment_regime',
                'status', 'description', 'domain', 'logo', 'legal_name', 'ignored',
                'email', 'bank_ids', 'classList', 'category'];
    ...
    async function getClearbitSuggestions(value) {
        const url = `https://api.astratech.com.do/dgii_ws/?term=${value}`;
    ...
    async function getOdooSuggestions(value, isVAT) {
        const method = isVAT ? 'read_by_vat' : 'dgii_autocomplete';
