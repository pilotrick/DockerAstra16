...
class PosOrder(models.Model):
    ...
    refund_payment_account_move_line_ids = fields.Many2many(
        "account.move.line")
    ncf_invoice_related = fields.Char(related="account_move.l10n_latam_document_number",
                                      string="NCF Factura")
    sale_fiscal_type = fields.Many2one(related="account_move.l10n_latam_document_type_id",
                                        string="Tipo",
                                        readonly=1)
    ncf_control = fields.Boolean(related="sale_journal.l10n_latam_use_documents")
        ...
        if self.ncf_control:
            if self.ncf:
                inv.update({
                    'reference': self.ncf,
                    'income_type': '01',
                    'sale_fiscal_type': self.partner_id.l10n_do_dgii_tax_payer_type
                })
            if self.return_order_id:
                inv.update(
                    {'origin': self.return_order_id.account_move.ref})
        return inv
    ...
    def get_from_ui(self, order):
        ...
        return {'ncf_invoice_related': pos_order.ncf_invoice_related,
                'sale_fiscal_type': pos_order.sale_fiscal_type.name,
                }
    ...
    def create_from_ui(self, orders, draft=False):
        ...
        #             record.account_move.assign_outstanding_credit(aml.id)
        #     if record.is_return_order:
        #         record.account_move.write({
        #             'origin_out': record.return_order_id.account_move.reference,
        #         })
        return res
    ...
    #             "invoice_id": [order.account_move.id, order.account_move.number],
    #             "amount_total": order.amount_total,
    #             "number": order.account_move.reference,
    ...
    #         if payment_name:
    #             out_refund_invoice = self.env["account.move"].sudo().search([
