...
class AccountInvoice(models.Model):
        ...
        for inv in self:
            if (
                ...
                if dates:
                    ...
                    inv.payment_date = (
                        max_date if max_date >= str(invoice_date) else invoice_date
                    )
            else:
    ...
    @api.depends(
        "line_ids.tax_ids", "line_ids.tax_base_amount", "state", "invoice_line_ids"
    )
                ...
                if inv.move_type == "in_invoice":
                    ...
                    for line in inv.line_ids:
                        ...
                        inv.withholded_itbis = (
                            inv._convert_to_local_currency(ritbis_amount) * -1
                        )
                        ...
                        inv.income_withholding = (
                            inv._convert_to_local_currency(isr_amount) * -1
                        )
    ...
    def _compute_is_exterior(self):
        for inv in self:
            inv.is_exterior = (
                True
                if inv.l10n_latam_document_type_id.l10n_do_ncf_type
    ...
    def norma_recompute(self):
        ...
        for k, v in self.fields_get().items():
            if v.get("store") and v.get("depends"):
                out_fields = ['attachment_ids', 'invoice_line_ids']
                if k in out_fields:
                    continue
    ...
    def _compute_taxes_fields(self):
        ...
        for inv in self:
            ...
            for line in inv.line_ids:
                ...
                inv.selective_tax = (
                    inv._convert_to_local_currency(isc_amount) * sign
                )
            ...
            inv.proportionality_tax = inv._convert_to_local_currency(
                sum(
                    inv.line_ids.filtered(
                        lambda line: line.account_id.account_fiscal_type
                        in ["A29", "A30"]
            ...
            inv.cost_itbis = inv._convert_to_local_currency(
                sum(
                    inv.line_ids.filtered(
                        lambda line: line.account_id.account_fiscal_type == "A51"
                    ).mapped("amount_currency")
                )
            )
