...
class SendNotification(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('validated', 'Validated'), ('schedule', 'Schedule'),
                             ('cancel', 'Cancel'), ('error', 'Error'), ('done', 'Done')], default="draft")
    message_to = fields.Selection([('all', 'To All'), ('specific', 'To Specific'), ('internal', 'Internal Users / Employee'), ('portal',
                                  'Portal Users / Ecommerce Customers'), ('public', 'Public Users / Non Registers Users')], default="all", string="Send Notification")
    specific_ids = fields.Many2many(
        'sh.push.notification', string="Register Id")
    log_historys = fields.One2many(
        'sh.notification.logger', 'base_config_id', string="Log History")
        ...
        if self.env.company.enable_web_push_notification:
            try:
                ...
                result = push_service.notify_multiple_devices(
                    registration_ids=registration_tokens, message_title=message_title, message_body=message_body, extra_notification_kwargs=extra_notification_kwargs)
