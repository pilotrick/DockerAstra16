...
class AdvanceProcurementProcess(models.Model):
    ...
    approve_id = fields.Many2one('res.users', string="Approver",
                                 ...
                                 domain=lambda self: [('groups_id', 'in',
                                    self.env.ref('setu_advance_reordering.group_advance_reordering_approved').ids)])
    ...
    buffer_stock_days = fields.Integer('Coverage days', help="Set days to maintain advance stock for products for next days")
    ...
    summary_ids = fields.One2many('advance.procurement.process.summary', 'procurement_process_id',
                                  string='Replenishment summary', help="Replenishment summary which will be carry forwarded in inter warehouse transfer or inter company transfer")
    ...
    procurement_total_volume = fields.Float("Total volume")
        ...
        if view_type == 'form':
            ...
            for node in doc.xpath("//field[@name='state']"):
                if approval_system_for_reorder:
                    node.set("statusbar_visible", "draft,inprogress,verified,waiting_for_approval,approved,reject,done,cancel")
                else:
    ...
    def onchange_buffer_security_days(self):
        ...
        for config in self.config_ids:
            config.advance_stock_start_date = (config.shipment_arrival_date + relativedelta.relativedelta(days=1)).strftime("%Y-%m-%d")
            config.advance_stock_end_date = (config.advance_stock_start_date + relativedelta.relativedelta(days=days)).strftime("%Y-%m-%d")
    ...
    def check_configuration_product(self):
        config_ids = self.config_ids.filtered(lambda x: x.warehouse_id.id != self.warehouse_id.id
                                                        and x.transit_days > 0)
        if not self.product_ids or not config_ids:
    ...
    def get_sales_data(self, config):
        ...
        if self.generate_demand_with == 'history_sales':
            return self.get_history_sales(products, warehouses, self.history_sale_start_date, self.history_sale_end_date)
        else:
    ...
    def prepare_reorder_line_vals(self, config, sales_data, generate_demand_with):
        ...
        for data in sales_data:
            ...
            wh_available = product.with_context(warehouse=config.warehouse_id.id).virtual_available#product.with_context({'warehouse': config.warehouse_id.id}).virtual_available
            ...
            if reorder_round_quantity and not demand_adjustment_qty % reorder_round_quantity == 0.0:
                if reorder_rounding_method == 'round_up' and not demand_qty == 0.0:
                    # c2 = (a + b) - (a % b);
                    demand_adjustment_qty = (demand_qty + reorder_round_quantity) - (demand_qty % reorder_round_quantity)
    ...
    def prepare_reorder_summary_vals(self):
        ...
        for product in self.product_ids:
            ...
            if lines:
                ...
                if (not main_wh_procurement_line and not other_wh_procurement_line) or (main_wh_procurement_line and not other_wh_procurement_line):
                    continue
                ...
                for line_id in other_wh_procurement_line:
                    line_id.wh_sharing_percentage = round((line_id.demand_adjustment_qty / other_wh_demand) * 100) or 0.0
                ...
                total_volume = order_qty * product.volume * product.weight
