...
class AdvanceReorderOrderProcess(models.Model):
    ...
    reorder_total_volume = fields.Float("Reorder Total Volume")
    ...
    def action_reorder_import_forecast_sale(self):
        self.ensure_one()
        action = self.env.ref('setu_advance_reordering.action_sale_forecast_auto_import_wizard').read()[0]
        ...
        for summary_line in self.summary_ids:
            ...
            if reorder_line:
                ...
                if summary_line.demanded_qty <= summary_line.vendor_moq:
                    ...
                    quantity = sum(reorder_line.mapped('demand_adjustment_qty'))
