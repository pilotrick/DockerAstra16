...
class SetuIntercompanyTransfer(models.Model):
    ...
    def create_sale_order(self):
        ...
        if not fpos:
            fpos = self.intercompany_channel_id and self.intercompany_channel_id.requestor_fiscal_position_id and \
                   self.intercompany_channel_id.requestor_fiscal_position_id or False
        so_vals = {
        ...
        if self.direct_deliver_to_customer:
            s_order = self.origin_order_id or order
            shipping_partner_id = s_order.partner_shipping_id.id
    ...
    def action_create_ict_invoices(self):
        ...
        if vb_process and len(self.purchase_ids.filtered(lambda x: x.invoice_status == 'to invoice')) > 0:
            ...
            channel_vendor_bill_journal = self.intercompany_channel_id and self.intercompany_channel_id.vendor_bill_journal_id and self.intercompany_channel_id.vendor_bill_journal_id.id or False
    ...
    def action_validate_ict_invoices(self):
        ...
        if invoices:
            in_invoices = invoices.filtered(lambda invoice: invoice.move_type == 'in_invoice' and invoice.amount_total >= 1)
            in_invoices and in_invoices.post()
            out_invoices = invoices.filtered(lambda invoice: invoice.move_type == 'out_invoice' and invoice.amount_total >= 1)
            out_invoices and out_invoices.post()
