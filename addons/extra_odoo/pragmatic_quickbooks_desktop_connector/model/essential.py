...
class ResPartner_Category_Inherit(models.Model):
    ...
    tag_type = fields.Selection(
        [('customer', 'Customer'), ('vendor', 'Vendor')], string='Tag Type')
                        ...
                        vals.update({
                            ...
                            'line_ids': [
                                (0, 0, {
                                    ...
                                    'discount_days': record.get('discount_days'),
                                })]
                        })
    ...
    def import_sales_tax(self, item_tax_data, company_id):
        ...
        if item_tax_data:
            for record in item_tax_data:
                ...
                if 'name' in record and record.get('name'):
                    tax_id = self.search(['&', ('name', '=', record.get(
                        'name')), ('amount', '=', record.get('amount'))], limit=1)
