                ...
                if line.purchase_line_id:
                    if line.purchase_line_id.order_id.purchase_manual_currency_rate_active:
                        price_unit = line.purchase_line_id.order_id.currency_id.round(
                            (line.purchase_line_id.price_unit) * line.purchase_line_id.order_id.purchase_manual_currency_rate)
                        rc.write({'unit_cost': price_unit, 'value': price_unit *
                                 rc.quantity, 'remaining_value': price_unit * rc.quantity})
        return rec

    def _prepare_account_move_vals(self, credit_account_id, debit_account_id, journal_id, qty, description, svl_id, cost):
        res = super(stock_move, self)._prepare_account_move_vals(
    ...
    def _prepare_account_move_line(self, qty, cost, credit_account_id, debit_account_id, svl_id, description):
        ...
        if self.purchase_line_id.order_id.purchase_manual_currency_rate_active:
            debit_value = self.purchase_line_id.order_id.currency_id.round(
                (self.purchase_line_id.price_unit * qty) * self.purchase_line_id.order_id.purchase_manual_currency_rate or 1)
            credit_value = debit_value

        if self.sale_line_id.order_id.sale_manual_currency_rate_active:
            credit_value = self.sale_line_id.order_id.currency_id.round(
                (self.sale_line_id.price_unit * qty) * self.sale_line_id.order_id.sale_manual_currency_rate or 1)
            debit_value = credit_value

        res = [(0, 0, line_vals) for line_vals in self._generate_valuation_lines_data(valuation_partner_id,
                                                                                      qty, debit_value, credit_value, debit_account_id, credit_account_id, svl_id, description).values()]
