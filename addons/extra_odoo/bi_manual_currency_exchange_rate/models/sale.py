...
class SaleOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        res.update({'manual_currency_rate_active': self.sale_manual_currency_rate_active,
...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _create_invoice(self, order, so_line, amount):
        ...
        if order.sale_manual_currency_rate_active:
            res.write({'manual_currency_rate_active': order.sale_manual_currency_rate_active,
...
class PricelistItem(models.Model):
    ...
    def _compute_price(self, product, quantity, uom, date, currency=None):
        ...
        if product_uom != uom:
            def convert(p): return product_uom._compute_price(p, uom)
        else:
            def convert(p): return p
        ...
        if self.compute_price == 'fixed':
            ...
            if manual_currency_rate_active:
                self = self.with_context(
                    manual_currency_rate_active=manual_currency_rate_active, manual_currency_rate=manual_currency_rate)
    ...
    def _compute_base_price(self, product, quantity, uom, date, target_currency):
        ...
        manual_currency_rate_active = product._context.get(
