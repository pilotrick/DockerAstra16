...
class AccountMove(models.Model):
    ...
    def _check_manual_currency_exchange_rate(self):
        for record in self:
            if record.apply_manual_currency_exchange:
        ...
        for move in self:
            ...
            for line in self.env['account.move.line'].search(domain):
                ...
                if line.currency_id == move.currency_id:
                    ...
                    if move.apply_manual_currency_exchange and move.manual_currency_exchange_rate:
                        amount = abs(line.amount_residual) * \
                            move.manual_currency_exchange_rate
                    else:
...
class AccountMoveLine(models.Model):
    ...
    def _compute_price_unit(self):
        for line in self:
            apply_manual_currency_exchange = line.move_id.apply_manual_currency_exchange
            manual_currency_exchange_rate = line.move_id.manual_currency_exchange_rate
            ...
            line.price_unit = line.product_id.with_context(apply_manual_currency_exchange=apply_manual_currency_exchange, manual_currency_exchange_rate=manual_currency_exchange_rate)._get_tax_included_unit_price(
    ...
    @api.depends('currency_id', 'company_id', 'move_id.date',
                 'move_id.apply_manual_currency_exchange', 'move_id.manual_currency_exchange_rate')
    def _compute_currency_rate(self):
        ...
        for line in self:
            if line.move_id.apply_manual_currency_exchange:
                rate = 1 / line.move_id.manual_currency_exchange_rate
    ...
    def _prepare_reconciliation_single_partial(self, debit_vals, credit_vals):
        ...
        def get_odoo_rate(vals):
            if vals.get('manual_currency_exchange_rate'):
                exchange_rate = vals.get('manual_currency_exchange_rate')
        ...
        if debit_vals.get('record') and debit_vals['record'].move_id.apply_manual_currency_exchange and debit_vals['record'].move_id.manual_currency_exchange_rate:
            debit_vals['manual_currency_exchange_rate'] = debit_vals['record'].move_id.manual_currency_exchange_rate
        if credit_vals.get('record') and credit_vals['record'].move_id.apply_manual_currency_exchange and credit_vals['record'].move_id.manual_currency_exchange_rate:
            credit_vals['manual_currency_exchange_rate'] = credit_vals['record'].move_id.manual_currency_exchange_rate
...
class ProductProduct(models.Model):
    ...
    def _get_tax_included_unit_price(self, company, currency, document_date, document_type,
        ...
        if product_taxes and fiscal_position:
            ...
            taxes_before_included = all(
                tax.price_include for tax in flattened_taxes_before_fp)
        ...
        apply_manual_currency_exchange = self._context.get(
            'apply_manual_currency_exchange')
        manual_currency_exchange_rate = self._context.get(
            'manual_currency_exchange_rate')

        if currency != product_currency:
            if apply_manual_currency_exchange:
                product_price_unit = product_price_unit * manual_currency_exchange_rate
            else:
