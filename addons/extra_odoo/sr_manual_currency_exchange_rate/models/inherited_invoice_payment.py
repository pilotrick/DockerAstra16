...
class account_payment(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if len(invoices or []) > 1:
            if all(inv.apply_manual_currency_exchange == False for inv in invoices):
                return rec
            if any(inv.apply_manual_currency_exchange == False for inv in invoices):
                ...
                rate = invoices[0].manual_currency_exchange_rate
                if any(inv.manual_currency_exchange_rate != rate for inv in invoices):
                    raise ValidationError(_("Selected invoice to make payment have not similer currency or currency rate is not same.\n Make sure selected invoices have same currency and same manual currency rate."))
        rec.update({
            'apply_manual_currency_exchange': invoices[0].apply_manual_currency_exchange,
            'manual_currency_exchange_rate': invoices[0].manual_currency_exchange_rate
        })
        return rec
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.apply_manual_currency_exchange:
            vals.update({'apply_manual_currency_exchange': self.apply_manual_currency_exchange, 'manual_currency_exchange_rate': self.manual_currency_exchange_rate})
        return vals
    
    @api.depends('can_edit_wizard', 'source_amount', 'source_amount_currency', 
    'source_currency_id', 'company_id', 'currency_id', 'payment_date',
    'apply_manual_currency_exchange', 'manual_currency_exchange_rate')
    ...
    def _get_total_amount_in_wizard_currency_to_full_reconcile(self, batch_result, early_payment_discount=True):
        ...
        if self.source_currency_id == self.currency_id:
            ...
            if self.apply_manual_currency_exchange and self.manual_currency_exchange_rate:
                return (self.source_amount_currency * self.manual_currency_exchange_rate), False
            else:
            ...
            if self.apply_manual_currency_exchange and self.manual_currency_exchange_rate:
                return abs(sum((aml.amount_residual / self.manual_currency_exchange_rate) for aml in batch_result['lines']
                )), False
            else:
                return abs(sum(
                    comp_curr._convert(
            ...
            if self.apply_manual_currency_exchange and self.manual_currency_exchange_rate:
                return self.source_amount / self.manual_currency_exchange_rate, False
            else:
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.apply_manual_currency_exchange and self.manual_currency_exchange_rate:
            conversion_rate = self.manual_currency_exchange_rate
        else:
...
class AccountPayment(models.Model):
    ...
    @api.depends('invoice_ids', 'amount', 'payment_date', 'currency_id', 'payment_type', 'manual_currency_exchange_rate')
    def _compute_payment_difference(self):
        return super()._compute_payment_difference()
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        if self.apply_manual_currency_exchange and self.manual_currency_exchange_rate > 0:
            currency_rate = self.company_id.currency_id.rate * self.manual_currency_exchange_rate
