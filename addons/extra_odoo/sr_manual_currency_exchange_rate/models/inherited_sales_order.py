...
class SaleOrderLine(models.Model):
    ...
    def product_uom_change(self):
        if self.order_id.active_manual_currency_rate:
            ...
            if self.order_id.apply_manual_currency_exchange and self.order_id.manual_currency_exchange_rate > 0:
                price_unit_calculate = self.price_unit = self.price_unit / \
                    self.order_id.manual_currency_exchange_rate / \
                    self.order_id.manual_currency_exchange_rate
                self.price_unit = price_unit_calculate
        ...
        if self.order_id.pricelist_id and self.order_id.partner_id:
            ...
            if self.order_id.apply_manual_currency_exchange and self. order_id.manual_currency_exchange_rate:
                self.price_unit = self.price_unit / self.order_id.manual_currency_exchange_rate / \
                    self.order_id.manual_currency_exchange_rate
